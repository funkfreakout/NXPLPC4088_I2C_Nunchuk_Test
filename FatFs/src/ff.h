...
typedef struct {
	...
	BYTE	win[_MAX_SS] __attribute__ ((aligned (4)));	/* Disk access window for Directory, FAT (and Data on tiny cfg) */
	
	/* Se alinea el buffer a una posición de palabra para que el GPDMA
     * pueda transferir hacia/desde el MCI palabra a palabra.
     */

     /* IMPORTANTE: la función f_mkfs utiliza un offset 446 dentro de win.
      ...
      * las transferencias DMA desde la fuente sean byte a byte o usar
      * la versión de sd_write sin DMA.
      */
} FATFS;
...
#endif
#if !_FS_TINY
	BYTE	buf[_MAX_SS] __attribute__ ((aligned (4)));	/* File data read/write buffer */
